...
if [ -z ${BLUEMIX_USER+x} ]; then echo 'Error: Environment variable BLUEMIX_USER is undefined.'; invocation_error=1; fi
# - CF_ORGANIZATION: IBM Cloud/Cloud Foundry organization name
if [ -z ${BLUEMIX_ORGANIZATION+x} ]; then echo 'Error: Environment variable BLUEMIX_ORGANIZATION is undefined.'; invocation_error=1; fi
# - CF_SPACE: IBM Cloud/Cloud Foundry space name
if [ -z ${BLUEMIX_SPACE+x} ]; then echo 'Error: Environment variable BLUEMIX_SPACE is undefined.'; invocation_error=1; fi
...
./Bluemix_CLI/bin/bluemix login --apikey $BLUEMIX_USER
